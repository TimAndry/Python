...
class UserManager(models.Manager):
    ...
    def validate_registration(self, post_data):
        ...
        if len(post_data['lname']) == 0 or NAME_REGEX.search(post_data['lname']):
        ...
        if len(post_data['password']) < 8 or not PASSWORD_REGEX.search(post_data['password']):
...
class User(models.Model):
    ...
    #objects = UserManager(), replaces objects call from user.objects.... to user.UserManager().
    objects = UserManager()
